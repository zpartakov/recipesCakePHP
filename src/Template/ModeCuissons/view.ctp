<div class="actions columns large-2 medium-3">
    <h3><?= __('Actions') ?></h3>
    <ul class="side-nav">
        <li><?= $this->Html->link(__('Edit Mode Cuisson'), ['action' => 'edit', $modeCuisson->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Mode Cuisson'), ['action' => 'delete', $modeCuisson->id], ['confirm' => __('Are you sure you want to delete # {0}?', $modeCuisson->id)]) ?> </li>
...
<div class="modeCuissons view large-10 medium-9 columns">
    <h2><?= h($modeCuisson->id) ?></h2>
    <div class="row">
        <div class="large-5 columns strings">
            <h6 class="subheader"><?= __('Lib') ?></h6>
            <p><?= h($modeCuisson->lib) ?></p>
            <h6 class="subheader"><?= __('Img') ?></h6>
            <p><?= h($modeCuisson->img) ?></p>
        </div>
        <div class="large-2 columns numbers end">
            <h6 class="subheader"><?= __('Id') ?></h6>
            <p><?= $this->Number->format($modeCuisson->id) ?></p>
            <h6 class="subheader"><?= __('Parent') ?></h6>
            <p><?= $this->Number->format($modeCuisson->parent) ?></p>
        </div>
    </div>
    <div class="row texts">
        <div class="columns large-9">
            <h6 class="subheader"><?= __('Rem') ?></h6>
            <?= $this->Text->autoParagraph(h($modeCuisson->rem)) ?>
        </div>
    </div>
</div>
<div class="related row">
    ...
    <?php if (!empty($modeCuisson->recettes)): ?>
    <table cellpadding="0" cellspacing="0">
        ...
        <?php foreach ($modeCuisson->recettes as $recettes): ?>
        <tr>
            <td><?= h($recettes->id) ?></td>
...
<div class="related row">
    ...
    <?php if (!empty($modeCuisson->recettes00)): ?>
    <table cellpadding="0" cellspacing="0">
        <tr>
            <th><?= __('Id') ?></th>
            <th><?= __('Prov') ?></th>
        ...
        <?php foreach ($modeCuisson->recettes00 as $recettes00): ?>
        <tr>
